...
interface SignMessageWithStarknetReactNextProps {
  chainId: constants.StarknetChainId | undefined
}

const SignMessageWithStarknetReactNext: FC<
  SignMessageWithStarknetReactNextProps
> = ({ chainId }) => {
  ...
  const message: UseSignTypedDataArgs = useMemo(() => {
    return {
      domain: {
        name: "Example DApp",
        chainId,
  ...
  const { signTypedDataAsync } = useSignTypedData({ params: message })
...
export { SignMessageWithStarknetReactNext }
